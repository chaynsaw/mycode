...
def marveleventcall(rand, keyhash, pubkey, lookmeup, orderBy=None):
    payload = {
        "ts": rand,
        "apikey": pubkey,
        "hash": keyhash
    }
    if orderBy in ["name", "modified", "-name", "-modified"]:
        payload.update({"orderBy": orderBy})

    r = requests.get(f"{API}", params=payload)  # send an HTTP GET to this location
...
def main():
    ...
    result = marvelcharcall(rand, keyhash, pubkey, args.hero, args.orderBy)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--orderBy', help='Order by name, modified, -name, -modified')
